...
def znormalize(x):
    mu_ = torch.mean(x, 1, keepdim=True)
...
class SGC(nn.Module):
    ...
    def __init__(self, njoint, ndim, nframe, nclass, nfeat):
        ...
        self.vit = ViT(njoints=njoint, num_classes=nclass, dim=nfeat, depth=6, heads=6, mlp_dim=njoint * nfeat,dropout=0.5, emb_dropout=0.5)
    ...
    def forward(self, x, batch_size, adj, degree, nclass, alpha, device):
        ...
        for sample_idx in range(batch_size):
            ...
            num_joint = blocks.shape[1]
            num_subject = int(num_joint / njoints)

            block_features = torch.zeros((num_block, num_subject, nclass)).float().to(device)
            # for each performing subject
            for sub_id in range(num_subject):
                for block_idx in range(num_block):
                    video_block = blocks[block_idx, sub_id * njoints: sub_id * njoints + njoints, :, :]
                    ...
                    block_features[block_idx, sub_id, :] = sgc_joint_vec_reg
            block_features_mean = torch.mean(block_features, 1)
            batch_block_output.append(block_features_mean)
            video_feat_aggr = torch.mean(block_features_mean, 0)
