...
from dataJEANIETE import *

import torchvision.transforms as transforms
from torch.utils.data import DataLoader, Dataset

from trainJEANIE import train
from testJEANIE import test

from modelJEANIE import get_model
...
nframe = 8  # number of frame per block (refer to dataloader my_collate func. for overlap & frame_per_block)
...
model = get_model(num_joints, ndim, nframe, num_class, nfeat, cuda)
...
PATH = 'models/' + dataset_name + '-nf' + str(nframe) + '-d' + str(degree) + '-' + str(CLASS_NUM) + 'way-' + str(
    SUPPORT_NUM_PER_CLASS) + 'shot_JEANIE' + '.pt'
